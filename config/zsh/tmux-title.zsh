...
#
# All snippets in this directory are concatenated into ~/.zshrc by
# home-manager, so a top-level `return` here would short-circuit the
# whole rc file. Wrap the hook setup in a guard instead.
if [[ -n "$TMUX" ]]; then
